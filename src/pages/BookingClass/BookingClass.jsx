...
const BookingClass = () => {
    ...
    const getSubjects = async () => {
        ...
        const array = results.filter((elem) => { 
            if (elem.teacherId.subjects.length > 0 && 
                new Date(elem.date+"T"+elem.time).getTime() >  new Date().getTime() )
             return true 
            })
        const newSubjects = []
        array.forEach((elem) => {
        ...
        setSubjects(newSubjects.filter((value, idx, self) => idx === self.findIndex((t) => 
    ...
    useEffect(() => {handleSubject()}, [booking])
